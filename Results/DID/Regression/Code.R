...
Reg2=function(dta,month,Size,Length,Output){
  ...
  results=modelsummary(list("No_FE"=result1,"FE"=result2,"TT"=result3,"2W"=result4),stars=TRUE,output=Output, gof_omit = 'DF|Deviance|R2|AIC|BIC|FE|Std.Errors')
  return(results)
}
...
Reg3=function(dta,month,Length,Output){
  ...
  dta=dta %>% distinct(registration_no,.keep_all = TRUE)
  result1=fepois(n~Post:Trt+Post:Trt:Agency_Size+Trt+Post,data=dta)
  result2=fepois(n~Post:Trt+Post:Trt:Agency_Size+Trt|floor_date(registration_start_date,"month"),data=dta)
  result3=fepois(n~Post:Trt+Post:Trt:Agency_Size+Post|estate_agent_name,data=dta)
  result4=fepois(n~Post:Trt+Post:Trt:Agency_Size|estate_agent_name+floor_date(registration_start_date,"month"),data=dta)
